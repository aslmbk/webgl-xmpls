...
export class Interpolant<T> {
  ...
  constructor(frames: Frame<number[]>[]) {
    ...
    this.interpolator = new THREE.LinearInterpolant(times, values, stride, this.frameBuffer);
  }
...
export class Vector3Interpolant extends Interpolant<THREE.Vector3> {
  ...
  get result() {
    return new THREE.Vector3(this.frameBuffer[0], this.frameBuffer[1], this.frameBuffer[2]);
  }
}
...
export class FloatInterpolant extends Interpolant<number> {
  ...
  toTexture() {
    ...
    for (let i = 1; i < this.frames.length - 1; i++) {
      const stepSize = (this.frames[i].time - this.frames[i - 1].time) / maxFrameTime;
    ...
    const texture = new THREE.DataTexture(data, width, 1, THREE.RedFormat, THREE.FloatType);
...
export class ColorInterpolant extends Interpolant<THREE.Color> {
  ...
  get result() {
    return new THREE.Color(this.frameBuffer[0], this.frameBuffer[1], this.frameBuffer[2]);
  }
  ...
  toTexture(alphaInterpolant: FloatInterpolant) {
    ...
    for (let i = 1; i < alphaInterpolant.frames.length - 1; i++) {
      const stepSize = (alphaInterpolant.frames[i].time - alphaInterpolant.frames[i - 1].time) / maxFrameTime;
      smallestStep = Math.min(smallestStep, stepSize);
    }
